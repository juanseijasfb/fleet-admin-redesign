...
export default function DriverTable({
	...
	const [modeSelectionTable, setModeSelectionTable] =
		useState<boolean>(showMultipleSelect);
	...
	const renderCell = React.useCallback(
		(driver: Driver, columnKey: ColumnKeys) => {
			...
			switch (columnKey) {
				case "firstName":
					return <span className="w-32 whitespace-nowrap">{cellValue}</span>;
				case "lastName":
					typeof cellValue === "string" ? cellValue.split(",")[0] || "" : "";
					return <span className="w-32 whitespace-nowrap">{cellValue}</span>;
				case "status":
					return (
						<Chip
				...
				<TableHeader columns={columns}>
					{(column) => (
						<TableColumn
							key={column.uid}
							align={
								["actions", "equipment", "status"].includes(column.uid)
									? "center"
									: "start"
							}
